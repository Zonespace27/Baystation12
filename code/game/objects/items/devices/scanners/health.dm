...
/proc/medical_scan_results(var/mob/living/carbon/human/H, var/verbose, var/skill_level = SKILL_DEFAULT)
	...
	if(H.should_have_organ(BP_BRAIN))
		...
		if(!brain || H.stat == DEAD || (H.status_flags & FAKEDEATH))
			brain_result = SPAN_SCAN_DANGER("none, patient is braindead")
		else if(H.stat != DEAD)
			if(H.has_brain_worms())
				brain_result = SPAN_SCAN_DANGER("ERROR - aberrant/unknown brainwave patterns, advanced scanner recommended")
			else
				if(skill_level < SKILL_BASIC)
					...
					switch(brain.get_current_damage_threshold())
						if(0)
							brain_result = "normal"
						if(1 to 2)
							brain_result = SPAN_SCAN_NOTICE("minor brain damage")
						if(3 to 5)
							brain_result = SPAN_SCAN_WARNING("weak")
						if(6 to 8)
							brain_result = SPAN_SCAN_DANGER("extremely weak")
						if(9 to INFINITY)
							brain_result = SPAN_SCAN_DANGER("fading")
						else
							brain_result = SPAN_SCAN_DANGER("ERROR - Hardware fault")
				else
					brain_result = SPAN_SCAN_DANGER("ERROR - Organ not recognized")
	else
		brain_result = SPAN_SCAN_DANGER("ERROR - Nonstandard biology")
	dat += "Brain activity: [brain_result]."

	if(H.stat == DEAD || (H.status_flags & FAKEDEATH))
		dat += SPAN_SCAN_WARNING("[b]Time of Death:[endb] [time2text(worldtime2stationtime(H.timeofdeath), "hh:mm")]")
	...
	if(H.should_have_organ(BP_HEART))
		...
		if(H.pulse() == PULSE_NONE)
			pulse_result = SPAN_SCAN_DANGER("[pulse_result]")
		else if(H.pulse() < PULSE_NORM)
			pulse_result = SPAN_SCAN_NOTICE("[pulse_result]")
		else if(H.pulse() > PULSE_NORM)
			pulse_result = SPAN_SCAN_WARNING("[pulse_result]")
	else
		pulse_result = SPAN_SCAN_DANGER("ERROR - Nonstandard biology")
	...
	if(H.should_have_organ(BP_HEART))
		if(H.get_blood_volume() <= 70)
			dat += SPAN_SCAN_DANGER("Severe blood loss detected.")
		var/oxygenation_string = "[H.get_blood_oxygenation()]% blood oxygenation"
		switch(H.get_blood_oxygenation())
			if(BLOOD_VOLUME_OKAY to BLOOD_VOLUME_SAFE)
				oxygenation_string = SPAN_SCAN_NOTICE("[oxygenation_string]")
			if(BLOOD_VOLUME_SURVIVE to BLOOD_VOLUME_OKAY)
				oxygenation_string = SPAN_SCAN_WARNING("[oxygenation_string]")
			if(-(INFINITY) to BLOOD_VOLUME_SURVIVE)
				oxygenation_string = SPAN_SCAN_DANGER("[oxygenation_string]")
	...
	switch(H.radiation)
		...
		if(1 to 30)
			dat += SPAN_SCAN_NOTICE("Patient shows minor traces of radiation exposure.")
		if(31 to 60)
			dat += SPAN_SCAN_NOTICE("Patient is suffering from mild radiation poisoning.")
		if(61 to 90)
			dat += SPAN_SCAN_WARNING("Patient is suffering from advanced radiation poisoning.")
		if(91 to 120)
			dat += SPAN_SCAN_WARNING("Patient is suffering from severe radiation poisoning.")
		if(121 to 240)
			dat += SPAN_SCAN_DANGER("Patient is suffering from extreme radiation poisoning. Immediate treatment recommended.")
		if(241 to INFINITY)
			dat += SPAN_SCAN_DANGER("Patient is suffering from acute radiation poisoning. Immediate treatment recommended.")

	// Traumatic shock.
	if(H.is_asystole())
		dat += SPAN_SCAN_DANGER("Patient is suffering from cardiovascular shock. Administer CPR immediately.")
	else if(H.shock_stage > 80)
		dat += SPAN_SCAN_WARNING("Patient is at serious risk of going into shock. Pain relief recommended.")
	...
	if(skill_level >= SKILL_BASIC)
		...
		if(H.getToxLoss() > 50)
			dat += SPAN_SCAN_GREEN("[b]Major systemic organ failure detected.[endb]")
	if(H.getFireLoss() > 50)
	...
	if(skill_level >= SKILL_BASIC)
		for(var/name in H.organs_by_name)
			...
			if(e.has_infected_wound())
				dat += SPAN_SCAN_WARNING("Infected wound detected in subject [limb]. Disinfection recommended.")
	...
	if(H.reagents.total_volume)
		...
		for(var/A in H.reagents.reagent_list)
			...
			if(R.scannable)
				...
				reagentdata[R.type] = SPAN_SCAN_NOTICE("[round(H.reagents.get_reagent_amount(R.type), 1)]u [R.name]")
			else
				unknown++
		if(reagentdata.len)
			print_reagent_default_message = FALSE
			. += SPAN_SCAN_NOTICE("Beneficial reagents detected in subject's blood:")
			for(var/d in reagentdata)
				. += reagentdata[d]
		if(unknown)
			print_reagent_default_message = FALSE
			. += SPAN_SCAN_WARNING("Warning: Unknown substance[(unknown>1)?"s":""] detected in subject's blood.")
	...
	if(ingested && ingested.total_volume)
		...
		for(var/datum/reagent/R in ingested.reagent_list)
			if(R.scannable)
				print_reagent_default_message = FALSE
				. += SPAN_SCAN_NOTICE("[R.name] found in subject's stomach.")
			else
				++unknown
		if(unknown)
			print_reagent_default_message = FALSE
			. += SPAN_SCAN_WARNING("Non-medical reagent[(unknown > 1)?"s":""] found in subject's stomach.")
	...
	if(H.chem_doses.len)
		...
		if(chemtraces.len)
			. += SPAN_SCAN_NOTICE("Metabolism products of [english_list(chemtraces)] found in subject's system.")
