...
/client/verb/who()
	...
	if(check_rights(R_INVESTIGATE, 0))
		for(var/client/C in GLOB.clients)
				...
				if(DEAD)
					if(isghost(C.mob))
						...
						if(O.started_as_observer)
							entry += " - [SPAN_WHO_OBSERVING("Observing")]"
						else
							entry += " - [SPAN_WHO_DEAD("<b>DEAD</b>")]"
					else
						entry += " - [SPAN_WHO_DEAD("<b>DEAD</b>")]"
			...
			if(age <= 1)
				age = SPAN_WHO_NEW_ACCOUNT("<b>[age]</b>")
			else if(age < 10)
				age = SPAN_WHO_NEWISH_ACCOUNT("<b>[age]</b>")

			entry += " - [age]"

			if(is_special_character(C.mob))
				entry += " - <b>[SPAN_WHO_ANTAGONIST("[C.mob.mind.special_role]")]</b>"
			if(C.is_afk())
