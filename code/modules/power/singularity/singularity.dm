		...
		if(M.stat == CONSCIOUS)
			if (istype(M,/mob/living/carbon/human))
				...
				if(istype(H.glasses,/obj/item/clothing/glasses/meson) && current_size != 11)
					...
					to_chat(H, SPAN_WARNING("You look directly into [src], but your eyewear does absolutely nothing to protect you from it!"))
		to_chat(M, SPAN_DANGER("You look directly into [src] and feel [current_size == 11 ? "helpless" : "weak"]."))
		M.apply_effect(3, EFFECT_STUN)
		for(var/mob/O in viewers(M, null))
			O.show_message(SPAN_DANGER("[M] stares blankly at [src]!"), 1)
