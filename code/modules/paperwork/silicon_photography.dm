...
/obj/item/device/camera/siliconcam/proc/injectmasteralbum(obj/item/photo/p) //stores image information to a list similar to that of the datacore
	...
	if(C.connected_ai)
		...
		to_chat(usr, SPAN_UNCONSCIOUS("Image synced to remote database, and can be printed from any photocopier."))//feedback to the Cyborg player that the picture was taken
	else
		to_chat(usr, SPAN_UNCONSCIOUS("Image recorded, and can be printed from any photocopier."))
	// Always save locally
	injectaialbum(p)
