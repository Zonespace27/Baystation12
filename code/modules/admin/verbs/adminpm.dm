...
/client/proc/cmd_admin_pm(var/client/C, var/msg = null, var/datum/ticket/ticket = null)
	...
	var/sender_message = SPAN_PM("<span class='out'>" + create_text_tag("pm_out_alt", "PM", src) + " to <span class='name'>[get_options_bar(C, holder ? 1 : 0, holder ? 1 : 0, 1)]")
	if(holder)
	...
	var/receiver_message = SPAN_PM("<span class='in'>" + create_text_tag("pm_in", "", C) + " <b>\[[recieve_pm_type] PM\]</b> <span class='name'>[get_options_bar(src, C.holder ? 1 : 0, C.holder ? 1 : 0, 1)]")
	if(C.holder)
	...
	for(var/client/X as anything in GLOB.admins)
		...
		if(X.key != key && X.key != C.key && (X.holder.rights & R_ADMIN|R_MOD))
			to_chat(X, SPAN_PM("[SPAN_OTHER("" + create_text_tag("pm_other", "PM:", X) + " <span class='name'>[key_name(src, X, 0, ticket)]")] to [SPAN_NAME("[key_name(C, X, 0, ticket)]")] (<a href='?_src_=holder;take_ticket=\ref[ticket]'>[(ticket.status == TICKET_OPEN) ? "TAKE" : "JOIN"]</a>) (<a href='?src=\ref[usr];close_ticket=\ref[ticket]'>CLOSE</a>): <span class='message linkify'>[msg]</span></span>"))
...
//		return
	...
	to_chat(src, SPAN_PM("[SPAN_OUT("" + create_text_tag("pm_out_alt", "PM", src) + " to <span class='name'>[sender]")]: <span class='message linkify'>[msg]</span></span>"))
	for(var/client/X as anything in GLOB.admins)
		if(X == src)
			continue
		if(X.holder.rights & R_ADMIN|R_MOD)
			to_chat(X, SPAN_PM("[SPAN_OTHER("" + create_text_tag("pm_other", "PM:", X) + " <span class='name'>[key_name(src, X, 0)]")] to [SPAN_NAME("[sender]")]: <span class='message linkify'>[msg]</span></span>"))
