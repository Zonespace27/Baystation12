...
/proc/_DB_staffwarn_record(var/ckey, var/reason)
	...
	if(!dbcon.IsConnected())
		to_chat(usr,SPAN_ERROR("Failed adding StaffWarn: db error"))
		return
...
/datum/admins/proc/DB_staffwarn_remove(var/ckey)
	...
	establish_db_connection()
	if(!dbcon.IsConnected())
		to_chat(usr,SPAN_ERROR("Failed removing StaffWarn: db error"))
		return 0

	var/DBQuery/query = dbcon.NewQuery("UPDATE erro_player SET staffwarn=NULL WHERE ckey='[dbckey]'")
	query.Execute()
	if(query.RowsAffected() != 1)
		to_chat(usr,SPAN_ERROR("StaffWarn unable to be removed from DB"))
		return 0
