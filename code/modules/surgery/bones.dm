...
/decl/surgery_step/bone/set_bone/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	if (affected.status & ORGAN_BROKEN)
		...
		user.visible_message(SPAN_NOTICE("\The [user] sets [bone]</span> <span class='warning'>in the WRONG place with \the [tool]."), \
			SPAN_NOTICE("You set [bone]</span> <span class='warning'>in the WRONG place with \the [tool]."))
		affected.fracture()
