...
/datum/ai_holder/simple_animal/goat/king/phase2/react_to_attack(atom/movable/attacker)
	...
	if(G.spellscast < 5)
		if(prob(5) && G.move_to_delay != 1) //speed buff
			...
			G.visible_message(SPAN_CULTANNOUNCE("\The [G]' eyes begin to glow ominously as dust and debris in the area is kicked up in a light breeze."))
			set_busy(TRUE)
...
/mob/living/simple_animal/hostile/retaliate/goat/king/phase2/proc/phase3_transition()
	...
	stun_chance = 10
	update_icon()
	visible_message(SPAN_CULTANNOUNCE("\The [src]' wounds close with a flash, and when he emerges, he's even larger than before!"))
...
/mob/living/simple_animal/hostile/retaliate/goat/king/proc/OnDeath()
	visible_message(SPAN_CULTANNOUNCE("\The [src] lets loose a terrific wail as its wounds close shut with a flash of light, and its eyes glow even brighter than before!"))
