...
/datum/hallucination/talking/start()
	...
	for(var/mob/living/talker in oview(holder))
		...
		if(prob(80))
			...
			to_chat(holder,"<span class='game say'>[SPAN_NAME("[display_name]")] [holder.say_quote(message)], [SPAN_MESSAGE("<span class='body'>\"[message]\"")]</span></span>")
		else
			to_chat(holder,"<B>[display_name]</B> points at [holder.name]")
			to_chat(holder,"<span class='game say'>[SPAN_NAME("[display_name]")] says something softly.</span>")
