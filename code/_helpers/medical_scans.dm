...
/proc/display_medical_data_body(var/list/scan, skill_level = SKILL_DEFAULT)
	...
	*/
	...
	for(var/list/E in scan["external_organs"])
		...
		row += "<tr><td>[E["name"]]</td>"
		if(E["is_stump"])
			row += "<td>[SPAN_BAD("Missing")]</td>"
			if(skill_level >= SKILL_ADEPT)
				row += "<td>[SPAN_BAD("[english_list(E["scan_results"], nothing_text = "&nbsp;")]")]</td>"
			else
				row += "<td>&nbsp;</td>"
		else
			row += "<td>"
			...
			if(skill_level < SKILL_ADEPT)
				if(E["brute_dam"])
					row += "[SPAN_BAD("Damaged")]<br>"
				if(E["burn_dam"])
					row += "[SPAN_AVERAGE("Burned")]</td>"
			else
				if(E["brute_dam"])
					row += "[SPAN_BAD("[capitalize(get_wound_severity(E["brute_ratio"], (E["limb_flags"] & ORGAN_FLAG_HEALS_OVERKILL)))] physical trauma")]<br>"
				if(E["burn_dam"])
					row += "[SPAN_AVERAGE("[capitalize(get_wound_severity(E["burn_ratio"], (E["limb_flags"] & ORGAN_FLAG_HEALS_OVERKILL)))] burns")]</td>"
			if(skill_level >= SKILL_ADEPT)
				row += "<td>"
	...
	/*
					...
					<tr><td>lungs</td><td>[SPAN_BAD("Severe</td><td>Decaying")]</td>
